...
private let disguiseLine = dLine(base: 1, tree: dNode(1, [dNode(2, [dNode(3)])]), rarity: .common) // 커먼 3형태
...
final class DittoRevealTests: XCTestCase {
    ...
    func testRevealAtFirstEvolution() async {
        ...
        s.applyUsage(accumulated)   // 라인 미로딩 중 적립 — 진화/리빌 보류
        XCTAssertEqual(s.state.active?.usedAtStage, accumulated)
        ...
        XCTAssertEqual(s.state.active?.usedAtStage, overflow, "첫 진화 초과분 이월")
    ...
    func testRevealRebasesDisguiseProfileAndLoadsDittoDetails() async {
        ...
        s.applyUsage(s.threshold + 1_000_000)
        ...
        XCTAssertEqual(profile?.growthTokens, PokemonBalance.graduationTotal(.rare) / 6)
        XCTAssertEqual(profile?.level, earnedLevel, "revealing Ditto preserves earned phase progress")
    }
    ...
    func testRevealClearsRepresentativePinnedToDisguiseAndFollowsDitto() async {
        ...
        let firstThreshold = PokemonBalance.phaseThreshold(rarity: .common, totalForms: 3, stageIndex: 0)
        s.applyUsage(firstThreshold + 1_000)
        await drainReveal(s)
    ...
    func testDelayedRevealDoesNotConvertSameBaseReplacementDisguise() async throws {
        ...
        let firstThreshold = PokemonBalance.phaseThreshold(rarity: .common, totalForms: 3, stageIndex: 0)
        let active = #"{"baseID":1,"pathIDs":[1],"plannedPathIDs":[1,2,3],"stageIndex":0,"usedAtStage":"#
            + "\(firstThreshold)"
            + #","rarity":"common","totalForms":3,"dittoDisguise":1}"#
...
#if false // PokéForge candy levels the selected trainee and never drives Ditto reveal progress.
    func testCandyPickerMatchesApparentPokemonButOnlySpendsEnoughToReveal() async throws {
        let s = seedDisguise(usedAtStage: 100_000_000)
        await drainReveal(s)
        ...
        XCTAssertEqual(s.state.active?.usedAtStage, 275_000_000)
    ...
    func testCandyBatchStopsExactlyAtRevealWithoutOverflow() async throws {
        let s = seedDisguise(usedAtStage: 25_000_000)
        await drainReveal(s)
        s.grantCandies(from: [], limitsReady: true)
