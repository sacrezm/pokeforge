...
struct CollectionView: View {
    ...
    private var visibleReceived: [TradePokemon] {
        let query = searchText.trimmingCharacters(in: .whitespacesAndNewlines)
        return received.filter { pokemon in
            (selectedRarity == nil || pokemon.rarity == selectedRarity) &&
                (!shinyOnly || pokemon.isShiny) &&
                (query.isEmpty || "#\(pokemon.speciesID)".contains(query) ||
                    pokemon.displayName.range(of: query, options: [.caseInsensitive, .diacriticInsensitive]) != nil)
        }
    }

    private var isCustomSortActive: Bool {
        navigation.collectionTab == .catchLog ? logSort != .recentFirst : dexSort != .numberAsc
    }
    ...
    private var filterToolbar: some View {
        HStack(spacing: 6) {
            ...
            Menu {
                if navigation.collectionTab == .catchLog {
    ...
    private var catchLog: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            if visibleEntries.isEmpty, visibleReceived.isEmpty {
                emptySearchResults
            } else {
                ScrollViewReader { proxy in
                    ScrollView {
                        LazyVStack(alignment: .leading, spacing: 8) {
                            ...
                            if !visibleReceived.isEmpty {
                                Text("Received in trades · \(visibleReceived.count)").font(.caption.weight(.semibold))
                                ForEach(visibleReceived) { pokemon in
                                HStack(spacing: 8) {
...
private struct DexGridView: View {
    ...
    var body: some View {
        ...
        let matcher = CompanionStore.DexSearchMatcher(searchText)
        let visible = store.sortDexSpecies(all.filter { species in
            (selectedRarity == nil || species.rarity == selectedRarity) &&
                (!shinyOnly || species.isShiny) &&
                (matcher.map { $0.matches(species) } ?? true)
        }, by: sortOption)
